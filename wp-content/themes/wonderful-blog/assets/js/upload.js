...
jQuery(function($){
  ...
  jQuery(document).on('click', 'input.select-img', function( event ){
    ...
    var Wonderful_Blog_Image = wp.media.controller.Library.extend({
      defaults :  _.defaults({
    ...
    wonderful_blog_file_frame = wp.media.frames.wonderful_blog_file_frame = wp.media({
      button: {
        text: jQuery( this ).data( 'uploader_button_text' ),
      },
      state : 'author-info-insert-image',
      states : [
        new Wonderful_Blog_Image()
        ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

    // When an image is selected, run a callback.
    wonderful_blog_file_frame.on( 'select', function() {

      var state = wonderful_blog_file_frame.state('author-info-insert-image');
    ...
    wonderful_blog_file_frame.open();
  });
});