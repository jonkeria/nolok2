...
if ( is_singular( 'post' ) ) {
	...
	if ( $query->have_posts() ) :
		$related_title = get_theme_mod( 'wonderful_blog_related_posts_title', __( 'Related Posts', 'wonderful-blog' ) );
		?>
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
						<div class="post-item post-grid">
							<div class="post-item-image">
								<a href="<?php the_permalink(); ?>"><?php wonderful_blog_post_thumbnail(); ?></a>
							</div>
							<div class="post-item-content">
								...
								<div class="post-content">
									<p><?php echo wp_kses_post( wp_trim_words( get_the_excerpt(), get_theme_mod( 'wonderful_blog_excerpt_length', 15 ) ) ); ?></p>
								</div><!-- post-content -->
							</div>
						</div>
					</article>
...
if ( is_front_page() ) {
	require get_template_directory() . '/inc/frontpage-sections/instagram.php';
}
...
<?php
$wonderful_blog_search = array( '[the-year]', '[site-link]' );
...
$copyright_text        = get_theme_mod( 'wonderful_blog_copyright_txt', $copyright_default );
$copyright_text        = str_replace( $wonderful_blog_search, $replace, $copyright_text );
?>
		...
		<div class="bottom-footer-info">
			<div class="site-info">
				<span>
					...
					<?php echo sprintf( esc_html__( 'Theme: %1$s By %2$s.', 'wonderful-blog' ), wp_get_theme()->get( 'Name' ), '<a href="' . wp_get_theme()->get( 'AuthorURI' ) . '">' . wp_get_theme()->get( 'Author' ) . '</a>' ); ?>
				</span>	
			</div><!-- .site-info -->
		</div>
	</div>
...
<?php if ( get_theme_mod( 'wonderful_blog_enable_scroll_to_top', true ) === true ) : ?>
